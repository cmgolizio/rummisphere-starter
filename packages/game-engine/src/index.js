...
export function sortRack(state, playerId, mode) {
  const player = getPlayer(state, playerId);
  ...
  const rackTiles = getRackTilesForPlayer(state, playerId);
  const colorRank = { red: 0, blue: 1, black: 2, orange: 3 };

  const sortedTiles = [...rackTiles].sort((a, b) => {
    ...
    const numberDifference = (a.number ?? 99) - (b.number ?? 99);
  ...
  const changed = rackTiles.some((tile) => {
    const sortedTile = sortedTileById.get(tile.id);

    return (
      sortedTile &&
      (tile.x !== sortedTile.x ||
        tile.y !== sortedTile.y ||
        tile.location !== sortedTile.location ||
        tile.ownerId !== sortedTile.ownerId)
    );
  });
  ...
  return {
    ...
    state: {
      ...state,
      tiles: state.tiles.map((tile) => sortedTileById.get(tile.id) || tile),
      lastError: null,
      version: state.version + 1,
      updatedAt: Date.now(),
    },
